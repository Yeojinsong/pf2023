...
<body>
	<div class="container-fluid cmsContainer">
		...
		<!-- EDIT WITHIN THIS TAG AND ... -->
			...
			<div class="row">
				<div class="col-12">
					<a href="#/" class="accordionButton" onclick="toggleAccordion('newDonationFact')">
					...
					<div class="accordionContent newDonationFact">
						<form action="includes/donationFacts/insert.php" method="POST">
							<fieldset>
								...
								<label>Fact:</label>
								<textarea id="newDonationFact" name="fact">
								</textarea>
								<script>
									CKEDITOR.replace("newDonationFact");
								</script>
								...
								<input class="formButton" name="donationFactAddSubmit" type="submit" value="Add"/>
							</fieldset>
						</form>
					</div>
				</div>
			...
			<div class="row">
				<div class="col-12">
					<a href="#/" class="accordionButton" onclick="toggleAccordion('updateDonationFact')">
						<div class="accordionHeading">
							<h3>Update</h3>
					...
					<div class="accordionContentOpen updateDonationFact row">

						<?php
						try {
							// write statement to get data from tbl_slider table
							$sql = "SELECT id, heading, fact, active FROM tbl_getInvolvedFact WHERE factType = 1;";
						...
						foreach ($resultSet as $row) {
							...
							<article class="col-12 col-lg-6">
								<form action="includes/donationFacts/update.php" method="POST">
									<fieldset id="form<?php echo $id; ?>" disabled>
										...
										<label>Fact:</label>
										<textarea id="donationFact<?php echo $id; ?>" name="fact">
											<?php echo $fact; ?>
										</textarea>
										<script>
											CKEDITOR.replace("donationFact<?php echo $id; ?>");
										</script>
									...
									<input id="submit<?php echo $id; ?>" class="formSubmit formButton" name="donationFactUpdateSubmit" type="submit" value="Submit"/>
									<input id="cancel<?php echo $id; ?>" class="formCancel cancelButton" type="button" onclick='disableForm(<?php echo $id; ?>)' value="Cancel"/>
								</form>

								<form action="includes/donationFacts/delete.php" id="deleteForm<?php echo $id; ?>" class="deleteForm" onsubmit="return checkDelete();" method="POST">
									<input type="hidden" name="id" value="<?php echo $id; ?>">
									<input type="submit" class="cancelButton" name="donationFactDeleteSubmit" value="Delete"/>
								</form>
								<hr><br>
							</article>
							<?php
