...
if (isset ($_POST['workingWithUpdateSubmit'])) {
	...
	try {
		if (!$_FILES['imgUpload']['name']=='') {
			...
			// if everything is ok, try to upload file
			    ...
			    if (move_uploaded_file($_FILES["imgUpload"]["tmp_name"], $target_file)) {
			    	$sql = "UPDATE tbl_workingWith SET name = :name, level = :level, logo = :logo, summary = :summary, url = :url, active = :active WHERE id = :id;";
			...
			$sql = "UPDATE tbl_workingWith SET name = :name, level = :level, summary = :summary, url = :url, active = :active WHERE id = :id;";
			$statement = $pdo->prepare($sql);
		}
		...
		echo "Error updating working With: " . $e->getMessage();
		exit();
	}
}
?>