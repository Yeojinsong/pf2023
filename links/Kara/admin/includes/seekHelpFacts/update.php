...
if (isset ($_POST['seekHelpFactsUpdateSubmit'])) {
	...
	try {
		if (!$_FILES['imgUpload']['name']=='') {
			...
			// if everything is ok, try to upload file
			    ...
			    if (move_uploaded_file($_FILES["imgUpload"]["tmp_name"], $target_file)) {
			    	$sql = "UPDATE tbl_seekHelpFact SET heading = :heading, img = :img, fact = :fact, active = :active WHERE id = :id;";
			...
			$sql = "UPDATE tbl_seekHelpFact SET heading = :heading, fact = :fact, active = :active WHERE id = :id;";
			$statement = $pdo->prepare($sql);
		}
		...
		echo "Error updating Seek Help Facts: " . $e->getMessage();
		exit();
	}
}
?>